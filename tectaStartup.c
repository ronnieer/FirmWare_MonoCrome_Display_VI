...
void fadeTECTA_InLargeCharactersOut(void){
    ...
    while(true){
        ...
        result = (uint16_t)(maxLuminanceDisplayStartup - (double)((maxLuminanceDisplayStartup - 100) * sin(3.14 / (double)(300) * repeats)));
...
void twinkleTECTA_InLargeYellowCharacters(void){
    ...
    colors = 0x03;
    while(true){
        result = (uint16_t)(maxLuminanceDisplayStartup - (double)((maxLuminanceDisplayStartup - 100) * (double)(1 - sin(3.14 / (double)(300) * repeats))));
        strcpy(string, "<=8945<=01");
        runHub75Row2Start(string, result);
        strcpy(string, ">?:;67>?23");
        runHub75Row3Start(string, result);            
        if(repeats > 300) break;
        repeats++;  
        vTaskDelay(10 / portTICK_PERIOD_MS);
    }   
}
...
void typeAplifierNumberInGreenCharacters(void){
    ...
    colors = 0x02;
    while(true){
        result = (uint16_t)(maxLuminanceDisplayStartup - (double)((maxLuminanceDisplayStartup - 100) * (double)(1 - sin(3.14 / (double)(220) * repeats))));
        isCharacterSizeSmall = true;
...
*/
    ...
    // repeats = 0;
    // while(true){
    //     result = 10000;//(uint16_t)(maxLuminanceDisplayStartup - (double)((maxLuminanceDisplayStartup - 100) * (1 - sin(3.14 / (double)(300) * 150))));
    //     isCharacterSizeSmall = true;
    //     strcpy(string, "Valve Pre-");
    //     runHub75Row1Start(string, result);   
    //     strcpy(string, "Amplfier  ");
